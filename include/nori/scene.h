 ...
 */
    ...
    public:
        ...
        // Added by Qiyuan
        Emitter *sampleEmitter(float x) const;

        void sampleEmitterInit();

        size_t getEmittersSize() const { return m_emitters.size(); }

        std::vector<Emitter *> getEmitters() const { return m_emitters; }

        // Simply test if the ray intersects an emitter. Assume there is only one emitter.
        Emitter *rayIntersectNonMeshEmitter(const Ray3f &ray) const {
            for (Emitter *em: m_emitters) {
                if (em->getShape()->rayIntersect(ray))
                    return em;
            }
            return nullptr;
        }
        ...
        /**
         * Computes the beam transmittance, which is the fraction of radiance transmitted
         * along the segment between the two points.
         * It accounts for both attenuation in participating media
         * as well as any surfaces that block the ray completely.
         * @param _ray The ray along which the beam transmittance is computed.
         * Note that ray.o, ray.d, ray.maxt and ray.m need to be set correctly.
         * @param sampler
         * @return Beam transmittance
         */
        Color3f Tr(const Ray3f &_ray, Sampler *sampler) const;
        bool rayIntersectTr(const Ray3f &_ray, Sampler *sampler, Intersection &its, Color3f &Tr) const;
