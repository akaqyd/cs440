...
template <typename _PointType> struct TBoundingBox {
    ...
    /// Added by Qiyuan Dong
    /// Create a bounding box from two positions and *calculate the minimum and maximum for each value*
    TBoundingBox(const PointType &_min, const PointType &_max, bool calculate) {
        if (calculate) {
            min[0] = std::min(_min[0], _max[0]);
            min[1] = std::min(_min[1], _max[1]);
            min[2] = std::min(_min[2], _max[2]);

            max[0] = std::max(_min[0], _max[0]);
            max[1] = std::max(_min[1], _max[1]);
            max[2] = std::max(_min[2], _max[2]);

        } else {
            throw NoriException("bbox.h: Calculate the minimum and maximum or not?!?");
        }
    }
