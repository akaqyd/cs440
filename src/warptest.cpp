...
static const std::string kWarpTypeNames[WarpTypeCount] = {
    ...
    "cosine_hemisphere", "beckmann", "microfacet_brdf", "hierarchical", "GGX"};
...
struct WarpTest {
    ...
    WarpTest(WarpType warpType_, float parameterValue_, BSDF *bsdf_ = nullptr,
        ...
        : warpType(warpType_), parameterValue(parameterValue_), bsdf(bsdf_), bRec(bRec_), xres(xres_), yres(yres_) {
    ...
    std::pair<bool, std::string> run() {
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == Square) {
                ...
                return Warp::squareToHierarchicalPdf(Point2f(x, y));
            else {
                x *= 2 * M_PI;
                y = y * 2 - 1;
    ...
    std::pair<Point3f, float> warpPoint(const Point2f &sample) {
        ...
        switch (warpType) {
            ...
            case Beckmann:
                ...
                break;
            case GGX:
                result << Warp::squareToGGX(sample, parameterValue);
                break;
            case Hierarchical:
                result << Warp::squareToHierarchical(sample);
                break;
            case MicrofacetBRDF: {
                BSDFQueryRecord br(bRec);
...
struct Arcball {
    ...
    Arcball(float speedFactor = 2.0f)
        : m_active(false), m_lastPos(nori::Vector2i::Zero()), m_size(nori::Vector2i::Zero()), m_quat(Quaternionf::Identity()), m_incr(Quaternionf::Identity()), m_speedFactor(speedFactor) {}
...
class WarpTestScreen : public Screen {
   public:
    ...
    void refresh() {
        ...
        m_parameterSlider->set_enabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == GGX);
        m_parameterBox->set_enabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == GGX);
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, {"Square", "Tent", "Disk", "Sphere", "Hemisphere (unif.)",
                                                "Hemisphere (cos)", "Beckmann distr.", "Microfacet BRDF", "Hierarchical", "GGX"});
