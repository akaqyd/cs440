...
bool Accel::rayIntersect(const Ray3f &_ray, Intersection &its, bool shadowRay) const {
    ...
    if (foundIntersection) {
        ...
        /* Qiyuan: Set mediumInterface for volumetric rendering */
        if (its.mesh->getMediumInterface().IsMediumTransition())
            its.mif = its.mesh->getMediumInterface();
        else
            its.mif = MediumInterface(_ray.m);

        /* Qiyuan: Normal mapping */
        if (its.mesh->getNormalTexture() != nullptr) {
            // Compute tangent space
            // Refer to https://marti.works/posts/post-calculating-tangents-for-your-mesh/post/
            Vector3f edge1 = p1 - p0;
            Vector3f edge2 = p2 - p0;

            Vector2f uv1 = UV.col(idx1) - UV.col(idx0);
            Vector2f uv2 = UV.col(idx2) - UV.col(idx0);
            ...
            if (!isnan(r) && !isinf(r)) {
                ...
                tangent.normalize();
                Vector3f bitangent = its.shFrame.n.cross(tangent).normalized();
                its.shFrame = Frame(bitangent, tangent, its.shFrame.n);


                Normal3f n_texture = its.mesh->getNormalTexture()->queryUV(its.uv);
                Normal3f n_mapping = its.shFrame.toWorld(n_texture);
                its.shFrame = Frame(n_mapping);
            }
        }

        /* Qiyuan: Color mapping */
        ...
        /* Qiyuan: Roughness mapping*/
        if (its.mesh->getRoughnessTexture() != nullptr) {
            Vector3f c = its.mesh->getRoughnessTexture()->queryUV(its.uv);
            assert(c.x() == c.y() && c.x() == c.z());
            its.roughness_texture = c.x();
            its.has_roughness_texture = true;
        }

        /* Qiyuan: Emitter mapping*/
        if (its.mesh->getEmitterTexture() != nullptr) {
            Vector3f c = its.mesh->getEmitterTexture()->queryUV(its.uv);
            its.emitter_texture = Color3f(c.x(), c.y(), c.z());
            its.has_emitter_texture = true;
        }
    }
